...
float g_uOffset = 0.0f;          // use of pass offset pos data to uniform var in shaders
...
void preDraw()
{
    ...
    UTILS_ASSERT((u_modelOffsetMatrixLocation >= 0), "uniform varialbe not found in shader");

    /* send the offset data to the shader */
    glUniformMatrix4fv(u_modelOffsetMatrixLocation, 1, GL_FALSE, &(translate[0][0]));
}
